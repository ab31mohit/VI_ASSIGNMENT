...
ADD_NEW_WP_AT = 10  # adding new waypoint after 10th waypoint
WP_PERPENDICULAR_DIST = 100  # add new waypoint at 100m perpendicular(altitude) distance

def get_planar_distance_metres(aLocation1, aLocation2):
    """
...
def get_vertical_distance_metres(aLocation1, aLocation2):
    """
    Returns the vertical distance in metres between two locational objects.
    """
    
    return abs(aLocation1.alt - aLocation2.alt)    # altitude difference in meters
...
def import_mission(filename, add_new_wp_at, perpendicular_dist):
    """
    Import mission from file and upload to vehicle
    """
    print(f"\n----Configuring Dynamic Mission----")
    ...
    # Add mission waypoints to the vehicle commands
    for i, line in enumerate(mission_items):
        ...
        if len(line_split) == 12:
            ...
            # adding 2 waypoints in this case (one is the original one at 10th index)
            # (second one is the extra waypoint added just perpendicular to 10th index by 100m)
            if i == add_new_wp_at:
                cmd = Command(0, 0, 0,
                            int(line_split[2]),    # frame
                            int(line_split[3]),    # command
                            ...
                            float(line_split[6]),  # param3
                            float(line_split[7]),  # param4
                            lat, lon, alt + perpendicular_dist)
                cmds.add(cmd)
...
def main():
    try:
        ...
        waypoint_count = import_mission('mission.txt', ADD_NEW_WP_AT, WP_PERPENDICULAR_DIST)
        ...
        while True:
            ...
            if (current_wp - last_waypoint) == 1:
                ...
                # check if the next waypoint is the one that was added dynamically (it is along vertical)
                if current_wp == ADD_NEW_WP_AT + 1:
                    distance = get_vertical_distance_metres(current_location, next_location)
                    current_speed = abs(vehicle.velocity[2])
                else:    
                    distance = get_planar_distance_metres(current_location, next_location)
